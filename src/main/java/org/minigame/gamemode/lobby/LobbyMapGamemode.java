...
public class LobbyMapGamemode implements MapGamemode<LobbyType> {
    ...
    public UnplayableMap getMap() {
        return MinigamePlugin.<UnplayableMap>getUniquie(MinigamePlugin.PLUGIN_ID + ":defaultminigamelobby").get();
    }
    ...
    public LobbyType getGamemode() {
        ...
        MinigamePlugin.getUniquie(GamemodeType.class).stream().forEach(g -> System.err.println("\t - " + g.getIdName()));
        return (LobbyType) opType.get();
    }
